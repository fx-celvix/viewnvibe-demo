...
export default function CheckoutPage() {
    ...
    const subTotalForPayment = itemSubTotal - discount + deliveryCharge;
    ...
    const handleProceed = async () => {
        ...
        try {
            ...
            const newOrder = {
                ...
                seen: false,
                appliedCoupon: appliedCoupon ? { code: appliedCoupon.code, discount: discount } : null,
            };
    ...
    if (!isClient || (cart.length === 0 && !isPlacingOrder)) {
        return (
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-4">
                                <div className="flex justify-between items-start">
                                    {selectedAddress ? (
                                        <>
                                            <div>
                                                <h2 className="text-sm font-semibold mb-2">Delivering to {selectedAddress.name}</h2>
                                            ...
                                            <button onClick={() => setIsAddressModalOpen(true)} className="text-senoa-green font-bold text-xs flex-shrink-0">
                                                CHANGE
                                            </button>
                                        </>
                                    ) : (
                    ...
                    <div className="bg-white border-t p-4 w-full max-w-md">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={handleProceed} disabled={isPlacingOrder} className="bg-senoa-green text-white font-bold py-3 px-6 rounded-md text-center flex items-center justify-center w-48 disabled:opacity-70">
                                {isPlacingOrder ? <Loader2 className="animate-spin h-5 w-5" /> : 'Place COD Order'}
                            </button>
                        </div>
                    </div>
                </footer>
